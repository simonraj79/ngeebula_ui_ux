# Verification record — React / PS1 release

Verified 17 September 2026 on Windows, Python 3.12 and Node 24. The current primary interface is React; historical Streamlit evidence is retained below and does not describe the deployed UI.

## Current release evidence

**232 tests passed** in 38.09 seconds. Two upstream Starlette/TestClient deprecation warnings remain. Python compileall and pip check passed.

- All 14 vendored upstream files match the pinned SHA-256 manifest, including the eight input CSVs. No source data was rewritten.
- Independent loader, rule mutation, solver, API and committed-CSV round-trip tests cover all 54 activities and 14 contracts.
- Validation rejects buffer-only intersections, incomplete workload, illegal co-sharing, invalid results, duplicate occupancy, inconsistent physical-night witnesses and malformed/nonfinite inputs.
- The imported-instance API test creates a separate dataset, runs the actual asynchronous solver, downloads exactly three CSVs, and revalidates those rows with the separately downloaded witness. Maintenance jobs remain empty.
- TypeScript/Vite build passes; npm production dependency audit reports zero known vulnerabilities at verification time.
...
Generated by `scripts/generate_ps1_submission.py` from the pinned inputs. Each scenario directory contains only RESULTS.csv, SCHEDULE_ACCESS.csv and SCHEDULE_OCCUPANCY.csv. Separate reports include input hashes, assumptions, scores, validation and physical-night witnesses. Tests reload and independently validate these exact files.

| Scenario | Activities | Access rows | Last week | ECLO rows | Excess possession nights | Total contract overrun days | Local objective |
|---|---:|---:|---:|---:|---:|---:|---:|
| A | 54 | 192 | 38 | 0 | 0 | 224 | 316.4 |
| B | 54 | 154 | 27 | 108 | 46 | 0 | 862.0 |
| C | 54 | 192 | 38 | 0 | 0 | 224 | 316.4 |

All three have zero locally observed hard violations and complete witness coverage. Objectives are comparable only within the same scenario. A/C continuing past week 30 assumes flat nominal supply continues. The official organiser validator is unavailable; neither this interpretation nor competitiveness against hidden instances is officially established. The supplied sample passes its reconstructable checks and reproduces 28 overrun days, but lacks the extra witness needed for complete local closure coverage.

The heuristic is not globally optimal; Scenario B still uses substantial ECLO and extra access. C can select its strict-supply incumbent because A plans are also admissible under C. The prototype supports another uploaded instance but does not claim hidden-instance performance guarantees or minimum-churn replanning.

# Historical Streamlit verification
...
## Stakeholder video (17 September 2026)

The separate Remotion project renders a 114-second 1080p walkthrough with real isolated-demo screenshots, narration, an original score, animated annotations and 40 subtitle cues. ESLint/TypeScript and helper compilation pass. Full MP4 decoding passes, and measured true peak is −2.08 dBTP. Scene-frame and final-file visual reviews corrected crops and callout placement. See [media verification and editorial boundaries](docs/video/VALIDATION.md). The deployed application source is unchanged by this production work.
